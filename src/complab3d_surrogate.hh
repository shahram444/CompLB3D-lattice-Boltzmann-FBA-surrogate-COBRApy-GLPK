...
/* One paragraph for the end of the run. Empty when no runtime network was ever used, so a run that
 * does not use this path prints nothing. */
inline std::string runtimeReport()
{
    const Runtime &R = runtime();
    if (R.evaluations == 0) return "";

    const double frac = 100.0 * (double) R.clamped / (double) R.evaluations;
    char b[768];
    std::string s = "  [SRG] runtime network: ";
    std::snprintf(b, sizeof(b), "%ld evaluation(s), %ld clamped to the training box (%.2f%%)\n",
    ...
    if (frac > 5.0) {
        std::snprintf(b, sizeof(b),
            ...
            "  [SRG] model. Widen the <inputN> ranges to cover the uptake rates this domain\n"
